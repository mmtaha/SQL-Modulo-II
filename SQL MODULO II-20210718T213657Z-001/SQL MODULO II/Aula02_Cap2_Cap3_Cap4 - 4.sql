...
--Um método para exportar um arquivo JSON é a utilização do BCP. No exemplo a seguir,
--será exportado o resultado da consulta no banco Pedidos, tabela TB_TIPOPRODUTO:
DECLARE @CMD VARCHAR(4000)
...
SELECT RESULTADO.*
FROM OPENROWSET (BULK 'C:\DADOS\ARQUIVOJSON.JSON-t', SINGLE_NCLOB) as j
...
-----CAP 6----------
--EM VIEWS NÃO É PERMITIDO O USO DE VARIAVEIS
...
--EM VIEWS NÃO É PERMITIDO O USO DE VARIAVEIS, SOMENTE É POSSIVEL UTILIZAR ORDER BY COM TOP
CREATE VIEW VW_EMP_2 WITH ENCRYPTION
AS
SELECT  CODFUN, 
		NOME, 
		DATA_ADMISSAO,
